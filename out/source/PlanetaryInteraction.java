...
float angleToStar; //angle from rocket to star
...
int asteroidspawnFrames; //spawn asteroid cooldown iterable variable
...
int loadingScreenIterator; //iterable variable used during startup
...
boolean enteredAmmoZone; //if entered ammozone this frame
boolean enteredBoosterZone; //if inside booster zone
boolean fetched; //if highscore got loaded
boolean frameSwitch; //for doing things every other frame
...
public void reset()
{
	...
	heading = 0; //direction of momentum angle 0-360
	angle = 0; //angle of rocket 0-360
	centerOffset = 15; //offset from center of mass
	angleToStar = 0; //rocket to star angle 0-360
	playfield = 800; //size of outer circle
	distanceToTip = 45; //offset distance from center of mass to tip of rocket

	//other floats
	rotationSpeed = 3; //amount to rotate rocket by per frame
	...
	lives = 3; //start with zero health points
	asteroidspawnFrames = 0; //iterable variable
	framesPlayingCrystalSong = 0; //ignore this
	spawnVal = 300; //spawn asteroid every five seconds in the beginning
	...
	//other booleans
	enteredAmmoZone = false; //update boolean to reset ammo amount
...
public void draw() {
	...
	//makes sure to never stroke drawn objects
	noStroke();
...
//-----------------------------------------------------------------------
//these are some useful math functions - trigonometry
...
public void StartScreen()
{
	...
	image(parallax4, 0 + (mouseX/24 - middle), 0 + (mouseY/24 - middle)); //offset image from center 1/24 of mouse pos
	image(parallax3, 0 + (mouseX/12 - middle), 0 + (mouseY/12 - middle)); //offset image from center 1/12 of mouse pos
	image(parallax2, 0 + (mouseX/8 - middle), 0 + (mouseY/8 - middle)); //offset image from center 1/8 of mouse pos
	image(parallax1, 0 + (mouseX/4 - middle), 0 + (mouseY/4 - middle)); //offset image from center 1/4 of mouse pos

	//header backdrop
	fill(0); //black
	rectMode(CENTER);
	rect(middle, 500, 600, 100); //makes sure stars never show up behind title
	...
	textAlign(CENTER);
	textSize(22);
	text("Click to continue", middle, 800); //start screen button
...
public void SinglePlayer()
{
	...
	if(asteroidspawnFrames >= spawnVal) //instantiate asteroid if iterable variable is over 300 frames
	{
		if(spawnasteroids == true) //only spawn if spawn asteroids is allowed 
		{
			...
			if(spawnVal > 90)
			{
				spawnVal -= 5; //instantiate asteroid five frames earlier for every asteroid spawned until asteroids spawn every 1.5 seconds
			}
		}
	}
	else
	{
		if(sceneIndex == 2) //make sure asteroids only spawn in single player scene
		{
	...
	if(fired == true) //checks if player hit space this frame
	{
		...
		fired = false; //player has fired this frame
	}
...
public void SinglePlayerLostScreen()
{
	...
	{
		//switch frame color every other frame
		if(frameSwitch)
		{
			background(0);
			fill(255, 255, 255);
	...
	if(mouseY > 770 && mouseY < 800 && mouseX < 612 && mouseX > 412) //check if mouse pos within button pos
	{
		...
		if(mousePressed){
			...
			sceneIndex = 2; //reset to main game mode
		}
	}
	else 
	{
...
class Asteroid {
	...
	float fallSpeed = 5; //speed of asteroid
	float asteroidSize; //size of asteroid
	...
	public void displayAsteroid()
	{
		...
		{   
			...
			asteroidRotation += asteroidRotationSpeed; //rotate asteroid per frame
			...
			//trigonometry math to calculate distance to asteroid from rocket
			...
			rocketToAsteroidDistance = (float)Math.sqrt(diagonalSquared); //pythagorean theorem
			...
			if(fired && alive == true)
			{
				...
				if(infiniteAmmo || ammo != 0)
				{
					...
					//keep rocket to asteroid angle within grad range 0-360
					...
					//set max angles to check if asteroid hit
		...
		{   //only do this once when the asteroid spawns
			...
			if(rotationDir == true)//reverse rotation speed
			{
...
class ExhaustParticle {
	...
	public void displayParticle()
	{
		...
		{
			if(aliveTime >= 50) //kill particle if it has existed for 50 frames - 5/6 of a second
			{
				aliveParticle = false;
			}
			...
			if(aliveParticle) //if particle is alive
			{
				...
				if(aliveTime <= 20)
				{
					fill(lerpColor((0xffff5900), (0xffffff00), (float)(aliveTime)/20)); //color 1-2
				}
				else if(aliveTime <= 40)
				{
					fill(lerpColor((0xffffff00), (0xffc9c9c9), (float)(aliveTime - 20)/20)); //color 2-3
				}
				else
				{
					fill(lerpColor((0xffc9c9c9), (color(201, 201, 201, 0)), (float)(aliveTime - 40)/10)); //color 3-4
				}
